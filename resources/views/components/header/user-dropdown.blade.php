<div class="relative" x-data="{
    dropdownOpen: false,
    userName: '{{ session('name') }}' || localStorage.getItem('user_name') || 'Usuario',
    userEmail: '{{ session('email') }}' || localStorage.getItem('user_email') || 'email@example.com',
    toggleDropdown() {
    ...
    init() {
        // Si no hay nombre en sesión, usar localStorage
        if (!this.userName || this.userName === '') {
            this.userName = localStorage.getItem('user_name') || 'Usuario';
        }
        if (!this.userEmail || this.userEmail === '') {
            this.userEmail = localStorage.getItem('user_email') || 'email@example.com';
        }
    }
    ...
    <button class="flex items-center text-gray-700 dark:text-gray-400" @click.prevent="toggleDropdown()" type="button">
        <span class="mr-3 overflow-hidden rounded-full h-11 w-11">
            <svg width="45" height="45" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M12 3.5C7.30558 3.5 3.5 7.30558 3.5 12C3.5 14.1526 4.3002 16.1184 5.61936 17.616C6.17279 15.3096 8.24852 13.5955 10.7246 13.5955H13.2746C15.7509 13.5955 17.8268 15.31 18.38 17.6167C19.6996 16.119 20.5 14.153 20.5 12C20.5 7.30558 16.6944 3.5 12 3.5ZM17.0246 18.8566V18.8455C17.0246 16.7744 15.3457 15.0955 13.2746 15.0955H10.7246C8.65354 15.0955 6.97461 16.7744 6.97461 18.8455V18.856C8.38223 19.8895 10.1198 20.5 12 20.5C13.8798 20.5 15.6171 19.8898 17.0246 18.8566ZM2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12ZM11.9991 7.25C10.8847 7.25 9.98126 8.15342 9.98126 9.26784C9.98126 10.3823 10.8847 11.2857 11.9991 11.2857C13.1135 11.2857 14.0169 10.3823 14.0169 9.26784C14.0169 8.15342 13.1135 7.25 11.9991 7.25ZM8.48126 9.26784C8.48126 7.32499 10.0563 5.75 11.9991 5.75C13.9419 5.75 15.5169 7.32499 15.5169 9.26784C15.5169 11.2107 13.9419 12.7857 11.9991 12.7857C10.0563 12.7857 8.48126 11.2107 8.48126 9.26784Z" fill="currentColor"></path></svg>',
        </span>

        <span class="block mr-1 font-medium text-theme-sm" x-text="userName"></span>
    ...
    <div x-show="dropdownOpen" x-transition:enter="transition ease-out duration-100"
        ...
        <!-- User Info -->
        <div>
            <span class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400" x-text="userName"></span>
            <span class="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400" x-text="userEmail"></span>
        </div>
