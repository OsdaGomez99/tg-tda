            ...
            <form action="{{ route('preguntas.update', $pregunta->id) }}" method="POST">
                ...
                <div>
                    ...
                    <input type="text" name="nombre" placeholder="Ingrese el título de la pregunta..."
                        ...
                        value="{{ old('nombre', $pregunta->nombre) }}" />
                    @error('nombre')
                ...
                <div class="mt-5">
                    ...
                    <textarea name="descripcion" placeholder="Ingrese una descripción para la pregunta..." rows="6"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $pregunta->descripcion) }}</textarea>
                    @error('descripcion')
                        <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                            <select name="tipo_tda"
                                ...
                                <option value="I" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                                    @if (old('tipo_tda') === 'I' || $pregunta->tipo_tda === 'I') selected @endif>
                                    Inatención
                                </option>
                                <option value="H" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                                    @if (old('tipo_tda') === 'H' || $pregunta->tipo_tda === 'H') selected @endif>
                    ...
                    <div>
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="estado" value="1" id="estado"
                                ...
                                @if (old('estado', $pregunta->estado)) checked @else checked @endif />
